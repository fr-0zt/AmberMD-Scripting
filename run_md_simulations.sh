...
usage() {
    echo
    echo "USAGE: run_md_simulations.sh [SYSTEM NAME] [CPU] [0/1] [GPU INDEX] [MD Start] [MD Stop]"
    echo
...
production_run() {
    if [ $# -lt 6 ]; then
        echo "Please specify the GPU index, the starting MD run, and the ending MD run."
        exit
    fi
    ...
    for dir in "003.equil" "004.md"; do
        cd $dir
